                    ...
                    User.findOne({
                        ...
                        if (!user) {
                            ...
                            newUser.save((err, user_new) => {
                                ...
                                if(key.admin.findIndex( e => e === userRecord.email)==-1){
                                    return res.json({ token: jwt.sign({ _id: user_new._id }, key.private) });
                                }else{
                                    return res.json({ token: jwt.sign({ _id: user_new._id, role: "admin" }, key.private) });
                                }
                            ...
                            if(key.admin.findIndex(e => e === userRecord.email)==-1){
                                ...
                                return res.json({ token: jwt.sign({ _id: user._id, role: "admin" }, key.private) });
                            }
                        }                      
                    });
                })
